...
function hashPin(pin, salt) {
  return scryptSync(String(pin), salt, 32).toString("base64url");
}
...
function makeSharedState(session, pin = generatePin()) {
...
export async function migrateLegacySession(store) {
  ...
  const state = await writeSharedState(makeSharedState(personal));
...
export async function verifyFamilyPin(request, store, pin) {
  ...
  const candidate = hashPin(String(pin ?? ""), state.pinSalt);
